...
import { useAddTopMovieMutation } from '@/services/topMovieApi'


function ChooseTopMoviesForm({ options, formTitle }) {

    const [addTopMovie, { isLoading, isError }] = useAddTopMovieMutation()
    const handleChooseFilm = async (event) => {
        event.preventDefault();
        const forma = new FormData(event.target)
        const data = Object.fromEntries(forma.entries())
        try {
            const response = await addTopMovie(data)
...
export default ChooseTopMoviesForm

...
function BannerCard({ id, movie,series }) {
    return (
        <div className='w-full rounded-md flex flex-col items-start gap-1 p-2 text-sm bg-indigo-300'>
            ...
            <h1>Nomi: <span className='font-semibold capitalize'>{(movie || series)?.name_uz}</span> </h1>
            <h2>Haqida: <span className='font-semibold capitalize'>{sliceText((movie || series)?.description_uz, 40)}</span> </h2>
